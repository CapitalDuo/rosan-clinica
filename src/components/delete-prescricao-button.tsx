...
import { excluirPrescricaoAction } from '@/app/(dashboard)/pacientes/[id]/prescricoes/actions'

export function DeletePrescricaoButton({
  prescricaoId,
  pacienteId,
}: {
  prescricaoId: string
  ...
  function handleDelete() {
    startTransition(async () => {
      await excluirPrescricaoAction(prescricaoId, pacienteId)
      setConfirming(false)
    })
  }
  ...
  return (
    <button
      ...
      className="p-1.5 rounded-[8px] text-muted hover:text-red hover:bg-red/10 transition-colors"
